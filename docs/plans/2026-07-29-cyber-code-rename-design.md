...
新增无外部依赖的 `internal/product` 包，集中定义产品名、命令名、配置目录、环境变量前缀、User-Agent 和默认系统身份。Agent、CLI、TUI、通知、MCP 和其他运行时代码不得重复硬编码产品名，而应从该包派生。

Go module path、发布产物名、文档命令示例和 CI YAML 无法从 Go 包动态求值，因此由发布残留扫描保证与品牌清单一致。品牌为编译期常量，不允许通过用户配置修改模型身份或产品安全边界。
...
先添加失败测试，覆盖统一品牌清单、Provider 请求中的系统身份、CLI/TUI 可见品牌、新配置路径和环境变量。完成机械重命名后运行全量测试、竞态检测、静态检查、漏洞扫描、覆盖率门禁，以及 Linux/Windows 交叉构建。最终扫描当前产品代码和文档，旧名称只能出现在历史来源说明、Anthropic/Claude 协议语义或测试中的明确负例。
