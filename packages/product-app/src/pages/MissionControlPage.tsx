...
export function MissionControlPage({ view, t, onDispatch, onReconnect, onDisconnect, clientId = 'web-client' }: MissionControlPageProps) {
  ...
  const sourceLabel = view.source
    ...
    : t.t('common.none');
  const isCyberAgent = view.source?.principal === 'cyber-agent';
  useEffect(() => {
    ...
    <div className="mission-canvas">
      ...
      <div className="mission-command-deck" data-testid="mission-command-deck">
        <header className="mission-header cyber-glass">
          ...
          {view.product.task && <div className="mission-task-summary"><strong>{view.product.task.status}</strong><span>{isCyberAgent ? 'Security Runtime - cyber-agent' : sourceLabel}</span>{isCyberAgent && <span>Authority: cyber-agent</span>}</div>}
