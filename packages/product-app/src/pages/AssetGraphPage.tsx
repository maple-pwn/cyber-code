...
export function AssetGraphPage({ product, t, onOpenReport }: { product: ProductState; t: Translator; onOpenReport?: () => void }) {
  ...
  const graph = useMemo(() => {
    const filtered = filterAssetGraph(createAssetGraph(product), { query, statuses: status === 'all' ? undefined : [status as 'active' | 'revoked' | 'deleted' | 'unknown'] });
    if (!product.report) return filtered;
    const reportId = `report-result:${product.report.id}`;
    const reportNode: AssetNodeState = { id: reportId, kind: 'report', label: t.t('assetGraph.reportResult'), status: 'active', attributes: { reportId: product.report.id, version: product.report.version }, provenance: { kind: 'human', annotationId: product.report.id, author: t.t('assetGraph.reportResultDetail') } };
    const anchor = filtered.nodes[filtered.nodes.length - 1];
    const reportEdge = anchor ? [{ id: `edge:${anchor.id}:${reportId}`, kind: 'produces-report', sourceId: anchor.id, targetId: reportId, directed: true, provenance: { kind: 'human' as const, annotationId: product.report.id, author: t.t('assetGraph.reportResultDetail') } }] : [];
    return { ...filtered, nodes: [...filtered.nodes, reportNode], edges: [...filtered.edges, ...reportEdge] };
  }, [product, query, status, t]);
